...
interface WorkflowContextType {
  ...
  endWorkflow: () => void;
  bindResearchQuery: (query: string) => void;
...
const LEGACY_ACTIVE_WORKFLOW_KEYS = ["ledger_active_workflow", "youtube_pro_active_workflow"] as const;
const ACTIVE_WORKFLOW_KEY = "cutroom_active_workflow";

function readActiveWorkflowId(): string | null {
  try {
    const current = localStorage.getItem(ACTIVE_WORKFLOW_KEY);
    if (current) return current;
    for (const legacyKey of LEGACY_ACTIVE_WORKFLOW_KEYS) {
...
function emptyResearchStub(query: string): CachedResearchData {
  return {
    query,
    totalResults: 0,
    videos: [],
...
function summaryFromState(state: WorkflowState): WorkflowHistorySummary | null {
  ...
  return {
    ...
    hasResearch: Boolean(state.cachedResearch?.videos?.length),
...
export function WorkflowProvider({ children }: { children: React.ReactNode }) {
  ...
  const queueSave = useCallback((nextState: WorkflowState) => {
    ...
    saveQueueRef.current = saveQueueRef.current.then(async () => {
      ...
      setHistoryError("Workflow progress could not be saved. Check that Cutroom can write local history.");
    });
  }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        let selectedRecord = records.find((record) => record.id === activeId) || records[0] || null;
        if (!selectedRecord) {
        ...
        if (!cancelled) {
          const legacy = readLegacyState();
          if (legacy) {
            const fallback = normalizeState(legacy);
          ...
          setHistoryError("Recent workflows are unavailable. New work will still save when storage recovers.");
        }
      } finally {
        if (!cancelled) {
          setHydrated(true);
  ...
  useEffect(() => {
    const flush = () => {
      ...
      if (!hydrated || !nextState.id || !nextState.createdAt || !nextState.updatedAt) return;
      queueSave(nextState);
    };
    const onVisibility = () => {
  ...
  const bindResearchQuery = useCallback((query: string) => {
    const trimmed = query.trim();
    if (!trimmed) return;
    setState((previous) => updateState(previous, {
      currentStep: "research",
      cachedResearch: emptyResearchStub(trimmed),
      idea: null,
    }));
  }, []);
  const setCachedResearch = useCallback((data: CachedResearchData) => setState((previous) => updateState(previous, { cachedResearch: data, currentStep: "research" })), []);
  const setIdeaData = useCallback((data: IdeaData) => setState((previous) => updateState(previous, { idea: data, currentStep: "research" })), []);
